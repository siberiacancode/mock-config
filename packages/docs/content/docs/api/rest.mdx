...
```ts
import { file, rest } from 'mock-config-server';

rest.get('/report', file('./files/report.pdf'), {
...
<TypeTable
  type={{
    ...
    match: {
      description: 'Matcher for this file response.',
      type: 'RestEntitiesByEntityName<Method>',
      typeDescriptionLink: '/docs/mocking-requests/references/Rest#RestEntitiesByEntityName'
    }
  }}
...
```ts
import { polling, rest } from 'mock-config-server';

rest.get(
  '/status',
  polling([{ response: { status: 'pending' }, time: 3000 }, { response: { status: 'done' } }])
);
```
...
<TypeTable
  type={{
    polling: {
      description:
        'Ordered list of polling items. Each item can be a response, handler, or file, with an optional `time` (ms) that controls how long the item stays active.',
