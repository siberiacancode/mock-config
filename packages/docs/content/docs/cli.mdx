...
| `--port`       | `-p`  | Specify the server port from config.                                 | `7777`                        |
...
Creates a mock configuration. It generates a `mock-server.config.(js|ts)` file with a sugar-style template (`mock`, `rest`, `graphql`, `ws`, …) in the current directory. An interactive flow lets you choose between TypeScript and JavaScript, select an API preset (REST, GraphQL, WebSocket, or Full API), and set basic server options like base URL, port and static path.
