...
- `polling()` for timed or request-based response sequences
- generator resolvers for request-by-request state

## Polling Helper

Use `polling()` when you want to describe an ordered sequence of GraphQL responses.
...
Use `polling()` when you need time windows. Use a generator when the sequence should advance once per request.
