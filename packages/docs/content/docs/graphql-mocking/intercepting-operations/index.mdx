...
Every GraphQL handler consists of two parts: an **identifier** and a **resolver**. Like in REST, a handler can also include a `match` condition and `settings` when you need more control.
...
The identifier decides which operations match the handler. The resolver decides what response to return.
...
Pass a response object when the result is fixed:
...
You can also pass the response inline without the `response` wrapper:
...
```ts
graphql.query<{ response: { data: { users: User[] } } }>('GetUsers', ({ entities }) => ({
  data: {
    users: [{ id: 1, name: 'John', role: entities.variables?.role }]
  }
}));
```
