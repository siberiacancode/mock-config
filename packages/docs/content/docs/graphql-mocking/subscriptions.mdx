...
For a single-event subscription, return a response directly or use the inline object form:

```ts
//[JAVA_COMMENT_REDACTED_f5aaf19d]
graphql.subscription('OnUserCreated', {
  data: { userCreated: { id: 1, name: 'John' } }
});

//[JAVA_COMMENT_REDACTED_e59c71bb]
graphql.subscription('OnUserCreated', {
  response: { data: { userCreated: { id: 1, name: 'John' } } }
});
```
...
```ts
graphql.subscription('OnNewMessage', {
  match: { variables: { channelId: 'general' } },
  response: { data: { newMessage: { text: 'Hello from #general' } } }
});

graphql.subscription('OnNewMessage', {
  match: { variables: { channelId: 'random' } },
  response: { data: { newMessage: { text: 'Hello from #random' } } }
});
```
...
graphql.subscription('OnUserCreated', {
  ...
  handler: async ({ next, complete, variables, setDelay }) => {
...
- Have no `settings` argument — delay can only be set via `setDelay` inside the handler
- Have no `queue` support — use `next` + `setDelay` in the handler to stream events
