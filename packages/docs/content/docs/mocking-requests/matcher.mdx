...
A matcher is the `match` field inside a config helper such as `rest.get`, `graphql.query`, `graphql.subscription`, or `ws.connection`.
...
export default mock(
  ...
  },
  {
    name: 'rest',
    configs: [
      rest.get<{ params: { id: string }; response: User }>('/users/:id', {
        match: {
          params: {
            id: '1'
          }
...
Use matchers when one endpoint or event channel must emulate more than one scenario.

They help you:

- keep real paths and operation names
- describe scenarios faster
- search and maintain configs more easily
- model UI states without inventing fake endpoints

If you need to reshape request or response data after matching, continue with [Interceptors](/docs/mocking-requests/references/Interceptors).
