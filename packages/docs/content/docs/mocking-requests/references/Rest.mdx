...
<TypeTable
  type={{
    ...
    routes: {
      description: 'One or more route responses or polling configs for this path',
...
You can pass a `polling` field with an array or generator of timed responses:
...
<TypeTable
  type={{
    settings: {
      ...
      type: (
        <span>
          <a href='#RestSettings' className='underline'>
            RestSettings
          </a>{' '}
          & {'{'} polling: true {'}'}
        </span>
    ...
    polling: {
      description: 'Sequence of timed responses for polling',
      type: (
        <span>
          Array{'<{'} time?: number; data:{' '}
          <a href='#RestDataResponse' className='underline'>
            RestDataResponse{'<'}Method{'>'}
          </a>{' '}
          {'}'} | {'{'} time?: number; file: string {'}>'} | Generator
        </span>
      ),
      required: true
    },
...
<TypeTable
  type={{
    ...
    polling: {
      description: 'Whether this route is polling (returns polling responses)',
      type: 'boolean'
    },
    status: {
