...
Some APIs are polled repeatedly by the client: a job status endpoint, a progress tracker, a notification feed. Mock Config supports this pattern with `polling()`: an ordered list of responses that the server cycles through as requests come in.
