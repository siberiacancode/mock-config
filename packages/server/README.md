...
fetch('http://localhost:7777/graphql', {
  method: 'POST',
  headers: {
    'name-header': 'Nursultan',
    'Content-Type': 'application/json'
            ...
            interceptors: {
              response: (data, { setHeader, getResponseHeader }) => {
                ...
                const updatedBuffer = Buffer.from(
                  buffer.toString('utf-8').replace('name', 'title')
                );
              ...
              request: ({ log }) => {
                log({
                  ...
                  options: {
                    ...
                    url: true //  url: 'http://localhost:7777/api/rest/posts/1'
                  } // }
                });
              },
              response: (data, { log }) => {
                log({
                  ...
                  options: {
                    // response get: http://localhost:7777/api/rest/posts/1 => 200
                    type: true,
                    statusCode: true,
                    method: true,
                    url: true
